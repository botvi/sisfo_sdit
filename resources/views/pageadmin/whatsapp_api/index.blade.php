                    ...
                    <div class="card border-top border-0 border-4 border-primary">
                        <div class="card-body p-5">
                            <div class="card-title d-flex align-items-center">
                                ...
                                <h5 class="mb-0 text-primary">Whatsapp Api</h5>
                            </div>
                            <hr>
                            <form action="{{ route('whatsapp-api.storeorupdate') }}" method="POST" class="row g-3">
                                @csrf
                                <div class="col-md-12">
                                    <label for="access_token" class="form-label">Account Token</label>
                                    <input type="text" class="form-control" id="access_token" name="access_token"
                                        value="{{ $whatsappApi->access_token ?? '' }}" required>
                                    <small class="text-danger">
                                        @foreach ($errors->get('access_token') as $error)
                                            <li>{{ $error }}</li>
                                        @endforeach
                                    </small>
                                </div>
