                ...
                <div class="col-xl-7 mx-auto">
                    ...
                    <div class="card border-top border-0 border-4 border-primary">
                        <div class="card-body p-5">
                            ...
                            <hr>
                            <form action="{{ route('master-kelas.update', $kelas->id) }}" method="POST" class="row g-3" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="col-md-12">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-control" id="wali_kelas_id" name="wali_kelas_id" required>
                                        @foreach ($waliKelas as $wali)
                                            <option value="{{ $wali->id }}" {{ $kelas->wali_kelas_id == $wali->id ? 'selected' : '' }}>
                                                {{ $wali->nama_wali_kelas }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <small class="text-danger">
                                        @foreach ($errors->get('wali_kelas_id') as $error)
                                            <li>{{ $error }}</li>
                                        @endforeach
                                    </small>
                                </div>
